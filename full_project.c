...
volatile unsigned char pressed = 1; //Nyomógomb segédváltozója
...
unsigned char n = 0; //A tömb elemeit számláló segédváltozó
volatile unsigned char transmit; //Jelzi, hogy megkezdődhet-e az adás a PC felé
...
ISR(TIMER2_COMPA_vect) //Az IT minden 20 us-bna lefut
{
	...
	if(count == 1000) //1000*20us = 20 ms után újrakezdjük a mérést
	{
		count = 0;
		front_count = 0;
		back_count = 0;
...
{
	switch(S) //A gomb megnyomása egy 1-0, majd 0-1 váltás egymásutánja amelyet a pressed változóval tartunk nyilván
	{
		case halt:
		if(pressed == 1)
		{
...
{   
    ...
    while (1) 
    {
		/*"Be-ki kapcsolás" kezelése*/
		if(BIT_IS_CLEAR(PINB,PB7)) //Ha a nyomógomb le van nyomva (logikai 0)
		{
			if(pressed == 1) //Ha először nyomtuk meg a gombot (halt állapotban bekapcsolás után)
			{
				S = still;
				pressed = pressed + 1;
			}
		...
		{
			case halt:
				if(n > 0) //Ha mentettünk már el állapotokat a data tömbbe, vagyis az autó befejezte a működést, akkor kimentjük az adatokat az EEPROM-ba
				{
			...
			case still:
				...
				cntr = 0;*/
				if(obst_front == 0) //Ha elől szabad az út, akkor előre megy
				{
					stopTurning();
					goForward();
					if(n < 20)
			...
			case forward:
				...
				if (obst_front == 1) //Ha a kocsi előtt akadály van, de ...
				{
					/*if(obst_left == 0)
					{
						turnLeft();
					...
					else //Ha egyik oldal sem szabad, tolasson
					{
						stop();
						goBackward();
						if(i < 20)
			...
			/*case backward: //Hátramenetbe csak akkor kerülhetünk, ha elől és a két oldalt is akadály van, így ahhoz hogy ebből az állapotból kikerüljünk addig kell hátramenni, amíg el nem tudunk kanyarodni
				if(obst_left == 0)
				{
					stop(); //Megáll a kocsi
					goForward();
